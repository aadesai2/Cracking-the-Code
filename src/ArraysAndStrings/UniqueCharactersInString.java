...
public class UniqueCharactersInString {
	...
	public static boolean uniqueChecker(String sentence){
		if(sentence.length() >256)  //If length of array > 256 --> not unique
			...
			return true;
		else if (sentence.length() == 2)	//If length of array = 2 --> check the 1st and 2nd character for uniqueness
		{
		...
		{
			for(int i = 0;i<sentence.length()-1;i++)   //check each char with it's next list of characters, 
													   //anywhere the same character is encountered, the string is not unique
			{
